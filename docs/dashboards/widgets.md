...
distinct values across the grid, and **values** are the aggregated measures. A
value with a **format** list colours its own cells (same layers as a table column,
scaled per value; cohort tables). Row filtering is the dashboard's job — use the
dashboard's `filters`, which feed the SQL.
  ...
  pivot:
    ...
    values:
      - field: sales
        ...
        label: Total Sales
        format:
          - backgroundColor: ["#FECACA", "#FEF08A", "#BBF7D0"]   # gradient over these cells
```
...
| `values` | `field`, `summarize`, `label`, `format` | Aggregated measures. `summarize` defaults to `sum`; `label` overrides the header. `format` is the same conditional-formatting layer list as a table column (each layer: optional `if`/`value`, `backgroundColor` single colour **or** low→high gradient array, `range`/`unit`, `textColor`, `bold`…), applied to this value's leaf cells and scaled per value — each value is coloured independently. |
